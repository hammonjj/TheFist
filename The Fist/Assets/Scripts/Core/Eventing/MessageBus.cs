...
namespace Bitbox.Core.Eventing
{
  /// <summary>
  ...
  public class MessageBus : MonoBehaviourBase
  {
    ...
    {
      if (gameObject.tag == Tags.SceneMessageBus)
      {
        LogInfo("Scene Message Bus Initialized - Sending Update Event");
        _globalMessageBus.Publish(new SceneMessageBusUpdatedEvent
        {
          MessageBus = this
        });
      }
